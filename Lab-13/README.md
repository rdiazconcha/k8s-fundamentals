Lab 13 - A bigger site
======================

Welcome to the deep end of the pool.  :D  We're going to take the site from Lab 05, and get it running on Kubernetes.
...
For this exercise, we're going to be using the `frontend:0.1` and `backend:0.1` images built in Lab 05.

1. Run `docker image list` and ensure both `frontend:0.1` and `backend:0.1` are present in the list.  If not, return to Lab 05 to build these images.

2. If you haven't already done it, run `docker push [REPOSITORY]/frontend:0.1` and `docker push [REPOSITORY]/backend:0.1` to push the images to your Docker Hub repository.
